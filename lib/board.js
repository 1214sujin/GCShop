...
module.exports = {
		...
		menu.menubar(req, (menubar) => {
			...
			db.query(sql1+sql2, [type_id,type_id], (err, subId) => {
				...
				db.query('select * from board b join person p on b.loginid=p.loginid\
						where b.type_id=? order by board_id desc limit ? offset ?',	// offset은 지나간 페이지수라고 생각하자
						[type_id, numPerPage, offs], (err, boards) => {
					var context = {
						menu: menubar,
						body: 'board.ejs',
						btname: subId[0][0],
	...
	detail: (req, res) => {
		...
		if (req.session.class == '01') {
			menu.menubar(req, (menubar) => {
				db.query('select * from board b join person p on b.loginid=p.loginid where b.board_id=?', [board_id], (err, board) => {
					context = {
						menu: menubar,
						body: 'boardCRU.ejs',
						board: board[0],
			...
			menu.menubar(req, (menubar) => {
				db.query('select * from board b join person p on b.loginid=p.loginid where b.board_id=?', [board_id], (err, board) => {
					...
					db.query('select loginid from board where loginid=?', [req.session.loginid], (err, loginid) => {
						if (req.session.loginid == loginid[0].loginid) context['accessible'] = 'Y'
						req.app.render('home', context, (err, html) => res.send(html))
					})
				})
			})
	...
	create_process: (req, res) => {
		...
		db.query('insert into board (type_id, loginid, password, title, date, content) values(?,?,?,?,?,?)',
				[post.type_id, post.loginid, post.password, post.title, date.now(), post.content], (err, result) => {
